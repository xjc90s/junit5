...
* When the timeout provided to `assertTimeout`, `assertTimeoutPreemptively` and test
  methods annotated with `@Timeout` is not a positive value less than 2^63^ nanoseconds
  (approximately 292 years) a `PreconditionViolationException` is thrown.
